...
router.post("/", async (req, res) => {
  try {
    ...
    const zone = await prisma.$transaction(async (tx) => {
      ...
      return tx.zone.create({ data: {
        canvasId, x: Number(x) || 0, y: Number(y) || 0,
        w: Math.max(Number(w) || 400, 120), h: Math.max(Number(h) || 300, 120),
        label: typeof label === "string" ? label : "", hue: Number.isInteger(hue) ? hue : 200,
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    const zone = await prisma.$transaction(async (tx) => {
      ...
      await lockCanvas(tx, existing.canvasId);
      return tx.zone.update({ where: { id: req.params.id }, data });
    });
    publish(zone.canvasId, { entity: "zone", action: "upsert", data: zone, clientId: req.header("x-client-id") });
...
router.delete("/:id", async (req, res) => {
  try {
    const zone = await prisma.$transaction(async (tx) => {
      ...
      if (!existing) throw new Error("Zone not found");
      await lockCanvas(tx, existing.canvasId);
      return tx.zone.delete({ where: { id: req.params.id } });
    });
