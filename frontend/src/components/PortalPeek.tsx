...
export function MiniField({ tasks, width = 200, height = 120 }: { tasks: Task[]; width?: number; height?: number }) {
  const t = useT();
  if (tasks.length === 0) {
    return <div className="flex items-center justify-center text-2xs text-nc-faint" style={{ width, height }}>{t("c.portalPeek.emptyBoard")}</div>;
  }
  const pad = 150;
    ...
    <svg width={width} height={height}>
      {tasks.map((t) => (
        <circle
          ...
          r={2}
          fill={t.done ? "var(--nc-text-faint)" : t.color || "#8b5cf6"}
        />
      ))}
    </svg>
  );
...
export function PortalPeek({ targetCanvasId, targetName }: { targetCanvasId: string; targetName: string }) {
  ...
  return (
    ...
    >
      ...
      {tasks === null ? (
        <div className="text-2xs text-nc-faint py-8 text-center">{t("c.portalPeek.peering")}</div>
      ) : (
        <>
          <MiniField tasks={tasks.filter((t) => !t.inbox && !t.archivedAt)} width={200} height={110} />
          <div className="text-2xs text-nc-faint mt-1">
