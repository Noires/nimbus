...
export function LedgerView({ canvasId, tasks, onOpenInspector }: { canvasId: string; tasks: Task[]; onOpenInspector: (task: Task) => void }) {
  ...
  return <section aria-labelledby="ledger-heading" className="space-y-4 p-3">
    ...
    {deleteCandidate && <div role="alertdialog" aria-modal="true" aria-labelledby="delete-view-heading" aria-describedby="delete-view-description" onKeyDown={(event) => { if (event.key === "Escape") setDeleteCandidate(null); }} className="rounded-nc-md border border-nc-danger/50 bg-nc-raised p-3">
