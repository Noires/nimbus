...
export function TableView({ onExit }: { onExit: () => void }) {
  ...
  const header = (key: SortKey, label: string) => (
    <th
      className="px-3 py-2 text-left text-2xs uppercase tracking-wider text-nc-faint cursor-pointer hover:text-nc-soft select-none whitespace-nowrap"
      onClick={() => {
  ...
  return (
    <div className="absolute inset-0 z-40 bg-nc-well/98 overflow-y-auto">
      <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-2.5 bg-nc-raised/95 backdrop-blur-md border-b border-nc-line-faint">
        <span className="text-xs text-nc-soft font-semibold">
          {tr("c.table.title", { count: rows.length })} <span className="text-nc-faint">{tr("c.table.hint")}</span>
        </span>
        <button onClick={onExit} className="text-nc-faint hover:text-nc-text text-sm">×</button>
      </div>
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-nc-line-faint">
            {header("done", "✓")}
            ...
            <th className="px-3 py-2 text-left text-2xs uppercase tracking-wider text-nc-faint">{tr("c.table.colTags")}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((t) => {
            ...
            return (
              <tr
                key={t.id}
                className="border-b border-nc-line-faint hover:bg-nc-fill-faint cursor-pointer transition-colors"
                onClick={() => jump(t)}
              >
                ...
                <td className={`px-3 py-1.5 text-xs ${t.done ? "line-through text-nc-faint" : "text-nc-text"}`}>
                  <span className="inline-block w-2 h-2 rounded-full mr-2" style={{ background: t.color }} />
                  {t.title}
                </td>
                <td className="px-3 py-1.5 text-xs text-nc-muted">{bubbleTitle.get(t.id) ?? "—"}</td>
                <td className={`px-3 py-1.5 text-xs whitespace-nowrap ${overdue ? "text-nc-danger" : "text-nc-muted"}`}>
                  {t.dueDate ? new Date(t.dueDate).toLocaleDateString(dateLocale()) : "—"}
                </td>
                <td className="px-3 py-1.5 text-xs">
                  <span className={
                    t.priority === "high" ? "text-nc-danger" : t.priority === "low" ? "text-nc-faint" : "text-nc-muted"
                  }>
                    {t.priority === "high"
